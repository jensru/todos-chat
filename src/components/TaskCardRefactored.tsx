...
export function TaskCardRefactored({ task, onUpdate, onDelete, isDragging = false, dragHandleProps, dragRef }: ITaskCardProps): React.JSX.Element {
  ...
  // Simple animation: Check if task was just loaded
  useEffect(() => {
    const isJustLoaded = (task as any).justLoaded === true;
    
    if (isJustLoaded) {
      console.log('🎨 Starting animation for:', task.title);
      setIsNew(true);
      // Fade out after 2 seconds
      const timer = setTimeout(() => {
        console.log('🎨 Fading out animation for:', task.title);
        setIsNew(false);
      }, 2000);
      
      return () => clearTimeout(timer);
    }
  }, [(task as any).justLoaded]);
  ...
  return (
    <Card 
      ref={dragRef}
      className={`transition-all duration-200 hover:shadow-sm ${task.completed ? 'opacity-60' : ''} py-2 gap-0 rounded-md ${isDragging ? 'opacity-50 shadow-lg' : ''} ${isNew ? 'bg-yellow-50' : ''}`}
    >
