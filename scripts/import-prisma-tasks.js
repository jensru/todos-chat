...
/**
 * Import der Prisma SQLite-Datenbank Tasks
 * Konvertiert das alte Prisma-Schema zu Supabase-Schema
 */
...
const prismaTasksRaw = fs.readFileSync('/tmp/prisma_tasks_raw.txt', 'utf8');
const prismaTasks = prismaTasksRaw.trim().split('\n').map(line => {
  ...
  return {
    ...
    globalPosition: parts[11] ? parseInt(parts[11]) : Date.now()
  };
});
...
// Mapping-Funktion für Prisma zu Supabase
function mapPrismaTaskToSupabase(prismaTask, userId) {
  ...
  return {
    ...
    notes: '', // Prisma hatte kein notes Feld
...
async function importPrismaTasks() {
  try {
    console.log('🚀 Starte Prisma SQLite Import...');
    ...
    console.log('\n📊 Prisma-Tasks Import-Statistiken:');
    ...
    if (futureTasks.length > 0) {
      console.log('\n🔮 Zukünftige Prisma-Tasks (ab 16.10.25):');
      futureTasks
    ...
    console.log('\n🎉 Prisma SQLite Import abgeschlossen!');
    console.log('📧 Login mit: jensrusi@gmail.com');
    console.log('🗄️ Diese Version kommt direkt aus der Prisma SQLite-Datenbank');
...
module.exports = { importPrismaTasks };
